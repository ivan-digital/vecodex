...
class VecodexIndex {
public:
    VecodexIndex(int dim, int segmentThreshold, IndexConfig<Index> config, std::tuple<ArgTypes...> args) : d_(dim), segmentThreshold_(segmentThreshold), config_(config), factory_(std::move(args)) {
    ...
    void mergeSegments() {
        ...
        VecodexSegment mergedSegment(factory_.create(config_));
...
private:
    ...
    VecodexSegmentFactory<Index, ArgTypes...> factory_;
    std::vector<VecodexSegment<Index>> segments_;
};
